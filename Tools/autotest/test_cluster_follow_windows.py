...
class ClusterFollowTest(object):
    ...
    def run(self):
        ...
        for c in self.copters:
            ...
            c.arm()
            c.takeoff_cmd(60)
        self.pump(lambda: all(c.rel_alt() > 50 for c in self.copters),
                  timeout=300, desc="copter-takeoff")
...
class Settings(object):
    ...
    speedup = 100           # requested SITL speedup for every vehicle
    ...
    # (must be comfortably under follow_time or the test cannot pass)
    min_hold_time = 30.0
