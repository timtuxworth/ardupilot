...
// Breach-inclusive twin of _find_fence_threats_NE() above: same categories, same
// ordering, same distance_line_to_*() calls, same inclusion-union and enabled-fences
// handling (that scoping lives inside AC_Fence::distance_line_to_*() itself, so it comes
// along for free) - the only difference is that a breached fence is never swapped out for
// FLT_MAX. See find_fence_clearance()'s header comment for why that suppression is wrong
// for recovery scoring even though it is correct for find_threats()'s threat dispatch.
//
// Also fills endpoint_m: whichever category produced the overall (start->end) minimum is
// re-queried as a degenerate zero-length segment at end_NE_cm alone - the exact same
// distance_line_to_*() call, not a second geometry implementation, so it inherits the same
// margin/enabled/union handling automatically. A second, independent pass over every
// category would cost roughly double _find_fence_threats_NE(); this costs at most one
// extra call, only for the category that already won.
float AP_OAScripting::_find_fence_clearance_NE(const Vector2f &start_NE_cm, const Vector2f &end_NE_cm,
                                                float lookahead_m, float &endpoint_m, OAObstacle &obstacle) const
{
    float distance_m = lookahead_m;
    endpoint_m        = lookahead_m;
#if AP_FENCE_ENABLED
    const AC_Fence *fence = AC_Fence::get_singleton();
    if (fence == nullptr || !fence->enabled()) {
        return distance_m;
    ...
    bool found = false;
    ObstacleType winner = ObstacleType::GENERAL;
    AC_PolyFenceType winning_inclusion_type = AC_PolyFenceType::POLYGON_INCLUSION;

    float distance_new_m = fence->distance_line_to_home_inclusion(start_NE_cm, end_NE_cm);
    if (distance_new_m < distance_m) {
        distance_m  = distance_new_m;
        winner      = ObstacleType::FENCE_HOME;
        found       = true;
    }
    distance_new_m = fence->distance_line_to_circle_exclusion(start_NE_cm, end_NE_cm);
    if (distance_new_m < distance_m) {
        distance_m  = distance_new_m;
        winner      = ObstacleType::FENCE_CIRCLE_EXCLUSION;
        found       = true;
    }
    // inclusion circles and polygons are one query: FENCE_OPTIONS INCLUSION_UNION makes
    // "inside any one of them" legal, which cannot be evaluated per category
    ...
    if (distance_new_m < distance_m) {
        distance_m              = distance_new_m;
        winner                  = (inclusion_type == AC_PolyFenceType::CIRCLE_INCLUSION)
                                        ? ObstacleType::FENCE_CIRCLE_INCLUSION
                                        : ObstacleType::FENCE_POLYGON_INCLUSION;
        winning_inclusion_type  = inclusion_type;
        found                   = true;
    }
    distance_new_m = fence->distance_line_to_polygon_exclusion(start_NE_cm, end_NE_cm);
    if (distance_new_m < distance_m) {
        distance_m  = distance_new_m;
        winner      = ObstacleType::FENCE_POLYGON_EXCLUSION;
        found       = true;
    }
    ...
    _populate_fence_obstacle(obstacle, winner);

    switch (winner) {
    case ObstacleType::FENCE_HOME:
        endpoint_m = fence->distance_line_to_home_inclusion(end_NE_cm, end_NE_cm);
        break;
    case ObstacleType::FENCE_CIRCLE_EXCLUSION:
        endpoint_m = fence->distance_line_to_circle_exclusion(end_NE_cm, end_NE_cm);
        break;
    case ObstacleType::FENCE_CIRCLE_INCLUSION:
    case ObstacleType::FENCE_POLYGON_INCLUSION: {
        AC_PolyFenceType endpoint_inclusion_type = winning_inclusion_type;
        endpoint_m = fence->distance_line_to_inclusion(end_NE_cm, end_NE_cm, endpoint_inclusion_type);
        break;
    }
    case ObstacleType::FENCE_POLYGON_EXCLUSION:
        endpoint_m = fence->distance_line_to_polygon_exclusion(end_NE_cm, end_NE_cm);
        break;
    default:
        break;
    }
...
// As find_fence_threats(), but breach-inclusive and returning two clearance values instead
// of one - see _find_fence_clearance_NE() above for the geometry and the header comment
// for why. For validate_horizontal_release()'s recovery scoring only; ordinary threat
// dispatch keeps using find_threats()/find_fence_threats(), whose breach-skip is
// unchanged. Unlike find_fence_threats(), start_loc == end_loc is accepted (a point-only
// "current clearance" query), so there is no start_NED_m == end_NED_m guard here.
bool AP_OAScripting::find_fence_clearance(const Location &start_loc, const Location &end_loc, float lookahead_m,
                                            float       &min_path_m,
                                            float       &endpoint_m,
                                            OAObstacle  &any_obstacle
                                            ) const
{
    ...
    const Vector2f end_NE_cm(end_NED_m.x * 100.0f, end_NED_m.y * 100.0f);
    OAObstacle obstacle {};
    min_path_m = _find_fence_clearance_NE(start_NE_cm, end_NE_cm, lookahead_m, endpoint_m, obstacle);
    if (min_path_m < lookahead_m) {
        any_obstacle = obstacle;
        return true;
    }
#else
